...
export async function invokeGroqText(prompt: string, systemPrompt?: string): Promise<string> {
  try {
    ...
    const completion = await groq.chat.completions.create({
      messages,
      model: process.env.GROQ_MODEL_ID_TEXT || "llama-3.3-70b-versatile", // Reliable text model
      temperature: 0.1,
    });
...
export async function invokeGroqVision(base64Image: string, prompt: string): Promise<string> {
  try {
    ...
    const completion = await groq.chat.completions.create({
      ...
      model: process.env.GROQ_MODEL_ID_VISION || "llama-3.2-90b-vision-preview", // Current vision model on Groq
      temperature: 0.1,
    });
